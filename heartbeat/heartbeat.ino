...
void sendHeartbeat() {
  ...
  if (current_jwt_token == "") {
    Serial.println("[Heartbeat] No JWT session token!");
    return;
  }

  HTTPClient http;
  String url = String(SERVER_URL) + "/iot/devices/heartbeat";
  http.begin(url);

  DynamicJsonDocument hbdoc(256);
  hbdoc["local_ip"] = WiFi.localIP().toString();
  hbdoc["bssid"] = WiFi.BSSIDstr();
  hbdoc["rssi"] = WiFi.RSSI();
  hbdoc["firmware_version"] = "2.0.0-esp32";
  String hbjson; serializeJson(hbdoc, hbjson);

  http.addHeader("Content-Type", "application/json");
  http.addHeader("Authorization", "Bearer " + current_jwt_token);

  int resp = http.POST(hbjson);
  Serial.print("[Heartbeat] HTTP code: "); Serial.println(resp);

  if (resp > 0) {
    String payload = http.getString();
    Serial.println("[Heartbeat] Server response:");
    Serial.println(payload);

    DynamicJsonDocument respDoc(256);
    DeserializationError err = deserializeJson(respDoc, payload);
    if (!err) {
      Serial.print("[Heartbeat] Status: "); Serial.println(respDoc["status"] | "");
      Serial.print("[Heartbeat] Timestamp: "); Serial.println(respDoc["timestamp"] | "");
    }
  } else {
    Serial.println("[Heartbeat] HTTP Error!");
  }
  http.end();
}
...
void loop() {
  if (current_jwt_token != "" && millis() - last_heartbeat > 60000) {
    sendHeartbeat();
